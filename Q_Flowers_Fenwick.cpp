...
void test_case() {
    ...
    for (size_t i = 0; i < (size_t)N; i++) cin >> height[i];
    for (int i = 0; i < N; i++) cin >> beauty[i];

    vector<ll> dp(N);
    ll mx = *max_element(all(height)) + 1;
    vector<ll> bit((size_t)mx);
    auto upd = [&](ll x, ll val) { for (; x <= mx; x += (x & -x)) { bit[x] = max(bit[x], val); } };
    auto qry = [&](ll x) { ll ans = 0; for (; x > 0; x -= (x & -x)) ans = max(ans, bit[x]); return ans; };
    ll ans = 0;
    for (int i = 0; i < N; i++) {
        dp[i] = qry(height[i]) + beauty[i];
        upd(height[i], dp[i]);
        ans = max(ans, dp[i]);
    }
    cout << ans << '\n';
